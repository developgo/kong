...
local function check_protocols_match_route(plugin, route)
  local array, set
  if #plugin > #route then
    set = Set(plugin.protocols)
    array = route.protocols
  else
    set = Set(route.protocols)
    array = plugin.protocols
  end
...
local function check_protocols_match_at_least_one_route(plugin, routes)
  for i = 1, #routes do
    if check_protocols_match_route(plugin, routes[i]) then
      return true
    end
  end
end
...
local function check_protocols_match(self, plugin)
  ...
  if type(plugin.route) == "table" then
    ...
    if route and not check_protocols_match_route(plugin, route) then
      local err_t = self.errors:schema_violation({
        protocols = "Plugin protocols must match the associated route's protocols"
      })
  ...
  if type(plugin.service) == "table" then
    local routes = self.db.routes:page_for_service(plugin.service) -- ignore error
    if routes and not check_protocols_match_at_least_one_route(plugin, routes) then
      local err_t = self.errors:schema_violation({
        protocols = "Plugin protocols must match at least one of the service's route's protocols"
      })
      return nil, tostring(err_t), err_t
    end
  end
